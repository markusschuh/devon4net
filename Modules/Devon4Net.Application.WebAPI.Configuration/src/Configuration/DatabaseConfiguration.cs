...
namespace Devon4Net.Application.WebAPI.Configuration.Configuration
{
    ...
    {
        public static void ConfigureDatabase<T,I>(this IServiceCollection services, ref IConfiguration configuration, string conectionStringName, DatabaseType databaseType, CosmosConfigurationParams cosmosConfigurationParams = null) where T : DbContext 
        {
            ...
            services.AddDbContext<DbContext, T>(options =>
             {
                 switch (databaseType)
                 {
                     ...
                     case DatabaseType.MariaDb:
                         options.UseMySql(connectionString.Value);
                         break;
